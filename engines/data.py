...
class DataManager:
    ...
    def get_dataset(self, df):
        ...
        df['token_id'] = df.token.map(lambda x: -1 if str(x) == str(np.nan) else self.token2id[x])
        df['label_id'] = df.label.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        # convert the data in matrix
        X, y = self.prepare(df['token_id'], df['label_id'])
...
class BertDataManager:
    ...
    def get_dataset(self, df):
        ...
        df['label_id'] = df.label.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        # convert the data in matrix
        X, y, att_mask = self.prepare(df)
        dataset = tf.data.Dataset.from_tensor_slices((X, y, att_mask))
        return dataset
