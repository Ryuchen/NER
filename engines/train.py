...
def train(configs, data_manager, logger):
    ...
    checkpoint_manager = tf.train.CheckpointManager(checkpoint, directory=checkpoints_dir, checkpoint_name=checkpoint_name, max_to_keep=max_to_keep)
    ...
    for i in range(epoch):
        ...
        for iteration in tqdm(range(num_iterations)):
            if configs.use_bert:
                X_train_batch, y_train_batch, att_mask_batch = data_manager.next_batch(X_train, y_train, att_mask_train, start_index=iteration * batch_size)
                # 计算没有加入pad之前的句子的长度
            ...
            with tf.GradientTape() as tape:
                logits, log_likelihood, transition_params = bilstm_crf_model.call(inputs=model_inputs, inputs_length=inputs_length, targets=y_train_batch, training=1)
            ...
            if iteration % configs.print_per_batch == 0 and iteration != 0:
                ...
                measures, _ = metrics(X_train_batch, y_train_batch, batch_pred_sequence, configs, data_manager, tokenizer)
                res_str = ''
        ...
        for iteration in tqdm(range(num_val_iterations)):
            if configs.use_bert:
                X_val_batch, y_val_batch, att_mask_batch = data_manager.next_batch(X_val, y_val, att_mask_val, iteration * batch_size)
            ...
            logits_val, log_likelihood_val, transition_params_val = bilstm_crf_model.call(inputs=model_inputs, inputs_length=inputs_length_val, targets=y_val_batch)
            ...
            measures, lab_measures = metrics(X_val_batch, y_val_batch, batch_pred_sequence_val, configs, data_manager, tokenizer)
